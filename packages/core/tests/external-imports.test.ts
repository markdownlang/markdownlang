...
describe("external file imports", () => {
  test("can call function from external file", () => {
    const md = readFileSync(
      resolve(examplesDir, "file-import/import-test.md"),
      "utf-8",
    );
    ...
    const output = interpret(
      ...
      "main",
      [],
      resolve(examplesDir, "file-import"),
    );
...
describe("remote URL imports", () => {
  ...
  test("nested relative imports within remote files (mocked fetch)", async () => {
    ...
    try {
      ...
      const remoteProgram = parse(
        `# entry\n\n[5](https://example.com/pkg/main.md#main)\n`,
      );
      const output = await interpretAsync(
        remoteProgram,
        "entry",
        [],
        process.cwd(),
  ...
  test("caching: multiple calls to same URL only fetch once (mocked fetch)", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        fetchCount,
        1,
        "Should only fetch the URL once due to caching",
      );
    } finally {
      globalThis.fetch = originalFetch;
    }
  ...
  test("real remote import from GitHub (mocked fetch)", async () => {
    const remoteUrl =
      "https://raw.githubusercontent.com/markdownlang/markdownlang/refs/heads/main/packages/examples/src/file-import/lib.md";
    const remoteLib = `# double\n\n1. n\n- result = n * 2\n\n**{result}**\n\n# greet\n\n1. name\n\n**Hello, {name}!**\n`;
