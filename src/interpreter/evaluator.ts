 ...
 */
export function evaluate(node: EvaluableExpression, runtime: RuntimeInterface, line?: number): RuntimeValue {
  switch (node.type) {
    case 'Literal':
    ...
    case 'TemplateLiteral':
      return evaluateTemplateLiteral(node as TemplateLiteralExpression, runtime, line);
    ...
    case 'CallExpression':
      ...
      throw new Error(`Call expressions not supported: ${(node as Expression & { callee?: { name: string } }).callee?.name}`);
...
function evaluateBinaryExpression(node: Expression, runtime: RuntimeInterface, line?: number): RuntimeValue {
  ...
  switch (binNode.operator) {
    ...
    case '+':
      ...
      return expectNumber(left, 'left operand of +', line) + expectNumber(right, 'right operand of +', line);
    case '-':
      return expectNumber(left, 'left operand of -', line) - expectNumber(right, 'right operand of -', line);
    case '*':
      return expectNumber(left, 'left operand of *', line) * expectNumber(right, 'right operand of *', line);
...
function evaluateUnaryExpression(node: Expression, runtime: RuntimeInterface, line?: number): RuntimeValue {
...
function evaluateTemplateLiteral(node: TemplateLiteralExpression, runtime: RuntimeInterface, line?: number): string {
  let result = '';
...
function evaluateMemberExpression(node: Expression, runtime: RuntimeInterface, line?: number): RuntimeValue {
